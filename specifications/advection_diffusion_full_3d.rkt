...
(define advection-diffusion-full-3d
  (hash
   'name "advection-diffusion-full-3d"
   'coordinates (list `x `y `z)
   'state (list `f)
   'state-assumptions (list)
   'parameters (list `ax `ay `az `Dxx `Dxy `Dxz `Dyx `Dyy `Dyz `Dzx `Dzy `Dzz)
   'parameters-assumptions (list `(> Dxx 0.0)
                                 `(> Dyy 0.0)
                                 `(> Dzz 0.0))
   'fluxes (list
            (list `(* ax f))
   ...
   'diffusive-fluxes (list
                      (list `(+ (* Dxx f_x) (* Dxy f_y) (* Dxz f_z)))
                      (list `(+ (* Dyx f_x) (* Dyy f_y) (* Dyz f_z)))
                      (list `(+ (* Dzx f_x) (* Dzy f_y) (* Dzz f_z))))
   )
  )
